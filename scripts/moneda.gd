...
func _on_body_entered(body: Node) -> void:
	if body.is_in_group("Player") and not ya_recolectada:
		ya_recolectada = true
		sonido.play()
		GameManager.contar_moneda() # Suma la moneda al contador global
		
		hide()
		collision.set_deferred("disabled", true)

# Guarda si la moneda existía o no en el instante que tocas el checkpoint
func guardar_estado_en_checkpoint() -> void:
	WorldState.set_state(scene_path, id_unico, "recolectada_en_checkpoint", ya_recolectada)

# Revive la moneda si no la habías recolectado cuando tocas el checkpoint
